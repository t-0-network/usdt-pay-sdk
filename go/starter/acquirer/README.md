...
|---|---|---|---|
| you → t-0 | `GetPaymentQuote` | Prices a fiat sale (fiat only) | `internal/get_payment_quote.go` |
| you → t-0 | `CreatePaymentIntent` | Opens an intent, returns the payment instructions (deposit options) | `internal/create_payment_intent.go` |
| you → t-0 | `SettlementReceived` | You confirm the fiat landed (fiat only) | `internal/settlement_received.go` |
| t-0 → you | `PaymentAuthorized` | Sale approved, release goods | `handler.go` |
| t-0 → you | `SettlementInitiated` | LP sent a bank transfer, pre-notice (fiat only) | `handler.go` |
| t-0 → you | `SettlementCompleted` | On-chain settlement verified (USDt only) | `handler.go` |
| t-0 → you | `PaymentExpired` | QR expired, cancel the sale | `handler.go` |
| t-0 → you | `PaymentFailed` | Deposit won't settle, cancel the sale | `handler.go` |
...
1. **2.1** Replace the demo sale in `cmd/main.go` — currency, amount and `paymentRef`
...
2. **2.2** Mint `paymentRef` and `idempotencyKey` when the sale is created rather
   ...
   Render each deposit option's `paymentUri` (from `GetUsdtOnChain().GetDepositOptions()`,
   once the instructions case says `USDT_ON_CHAIN`) as a QR image **as-is** — it is
...
Implement the callbacks in `handler.go`.
...
1. **4.1** When the LP's transfer shows up on your bank statement, match it on
   `(lpId, bankTransferRef)` and call `ConfirmSettlement(...)`. The intent
...
**Inbound — your callback handler is a plain struct.** Construct it and call the
method directly with `connect.NewRequest`:
...
```
cmd/main.go              # entry point, phases in order
config.go                # what .env supplies
handler.go               # PaymentAuthorized, SettlementInitiated,
